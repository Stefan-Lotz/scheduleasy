...
app.post('/login', async (req, res) => {
    ...
    var passOk = "";
    password === "" ? passOk = "" : passOk = bcrypt.compareSync(password, userDoc.password);
    if(passOk) {
...
app.get('/schedule/:url', async(req, res) => {
    ...
    const scheduleDoc = await ScheduleModel.findOne({ url }).populate('author', ['username']).populate('messages'); // Populate messages
    res.json(scheduleDoc);
});
